...
final class NCOMAppLibrary: ObservableObject {
    ...
    init() {
        if let data = UserDefaults.standard.data(forKey: key), let decoded = try? JSONDecoder().decode([NCOMAppItem].self, from: data) { apps = decoded }
        if apps.isEmpty {
            apps = [
                NCOMAppItem(id: UUID(), name: "AI Chat", subtitle: "Normal AI conversation", symbol: "bubble.left.and.bubble.right.fill", category: "AI", iconFile: nil),
            ...
            ScrollView {
                LazyVGrid(columns: columns, spacing: 14) {
                    ...
                    Button { showBuilder = true } label: {
                        VStack(spacing: 9) { Image(systemName: "plus").font(.title2.weight(.bold)); Text("Build App").font(.system(.footnote, design: .rounded).weight(.semibold)) }
                        .frame(maxWidth: .infinity).aspectRatio(0.92, contentMode: .fit)
...
private struct NCOMAppTile: View {
    ...
    var body: some View {
        VStack(spacing: 9) {
            ZStack { RoundedRectangle(cornerRadius: 24, style: .continuous).fill(.regularMaterial); Image(systemName: app.symbol).font(.system(size: 28, weight: .medium)).foregroundStyle(.primary) }
        ...
        NavigationStack {
            Form {
                ...
                Section("Icon") {
                    ...
                    Text("Artwork can come from the iOS Files picker, including files transferred from the PC.").font(.footnote).foregroundStyle(.secondary)
                }
                Section {
                    Button("Create NCOM App") {
                        library.add(NCOMAppItem(id: UUID(), name: name.isEmpty ? "New NCOM App" : name, subtitle: description.isEmpty ? "Custom NCOM app" : description, symbol: symbol.isEmpty ? "app.fill" : symbol, category: category, iconFile: nil)); dismiss()
