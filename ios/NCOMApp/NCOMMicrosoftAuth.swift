...
final class NCOMKeychain {
    ...
    func save(_ value: Data, key: String) {
        ...
        var item = base; item[kSecValueData as String] = value; item[kSecAttrAccessible as String] = kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly
        SecItemAdd(item as CFDictionary, nil)
    }
    func load(_ key: String) -> Data? {
        var query: [String: Any] = [kSecClass as String: kSecClassGenericPassword, kSecAttrAccount as String: key, kSecReturnData as String: true, kSecMatchLimit as String: kSecMatchLimitOne]
...
@MainActor
final class NCOMMicrosoftAuth: NSObject, ObservableObject {
    ...
    override init() { super.init(); authenticated = NCOMKeychain.shared.load(accessKey) != nil }

    func signIn(anchor: ASPresentationAnchor) async throws {
        guard let clientID = Self.clientID, !clientID.isEmpty else { throw NSError(domain: "NCOM.Microsoft", code: 1, userInfo: [NSLocalizedDescriptionKey: "MICROSOFT_CLIENT_ID is not configured in this private build."]) }
        ...
        let callback = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<URL, Error>) in
            let s = ASWebAuthenticationSession(url: components.url!, callbackURLScheme: "com.ncom.ai") { url, error in
                if let url { continuation.resume(returning: url) } else { continuation.resume(throwing: error ?? NSError(domain: "NCOM.Microsoft", code: 2)) }
            }
        ...
        guard let items = URLComponents(url: callback, resolvingAgainstBaseURL: false)?.queryItems,
              ...
              let code = items.first(where: { $0.name == "code" })?.value else { throw NSError(domain: "NCOM.Microsoft", code: 3, userInfo: [NSLocalizedDescriptionKey: "Invalid Microsoft callback."]) }
    ...
    private func exchange(code: String, verifier: String, clientID: String) async throws {
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else { throw NSError(domain: "NCOM.Microsoft", code: 4, userInfo: [NSLocalizedDescriptionKey: "Microsoft token exchange failed."]) }
    ...
    private struct TokenResponse: Decodable { let accessToken: String; let refreshToken: String?; enum CodingKeys: String, CodingKey { case accessToken = "access_token"; case refreshToken = "refresh_token" } }
}

extension NCOMMicrosoftAuth: ASWebAuthenticationPresentationContextProviding {
...
struct NCOMOutlookProgram {
    static func sendMail(token: String, to: String, subject: String, body: String) async throws {
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else { throw NSError(domain: "NCOM.Outlook", code: (response as? HTTPURLResponse)?.statusCode ?? -1) }
    }
}
